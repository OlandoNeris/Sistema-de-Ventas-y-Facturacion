...
<?php 
	...
	// validar que los campos enviados por la url no vengan vacios o con valores incorrectos 
	...
	if (isset($_REQUEST['fecha_desde']) || isset($_REQUEST['fecha_hasta'])) 
	{
...
// ----------------------------------------------------------------------------
	...
	if (!empty($_REQUEST['fecha_desde']) && !empty($_REQUEST['fecha_hasta'])) {
		...
		// armo las cadenas para la consulta 

		}else if($fecha_desde == $fecha_hasta){
			$where = "fecha LIKE '$fecha_desde%'";
			$buscar = "fecha_desde=$fecha_desde&fecha_hasta=$fecha_hasta";
		}else{
			$f_desde = $fecha_desde.' 00:00:00';
			$f_hasta = $fecha_hasta.' 23:59:59';
			$where = "fecha BETWEEN '$f_desde' AND '$f_hasta'";
			$buscar = "fecha_desde=$f_desde&f_hasta=$fecha_hasta";
		}
	...
	<section id="container_listado">
		...
		<table>
			...
			<?php 
				...
				$sql_registro = mysqli_query($conn, "SELECT COUNT(*) as total_registros FROM factura WHERE $where ");
